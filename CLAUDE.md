...
- `@chat/` — 클라이언트 사이드 렌더링 채팅 인터페이스 (모바일 `w-full`, `web`(1055px) 이상 475px 고정)
- `@side/` — 서버 사이드 렌더링 프로필/연락처 섹션 (1055px 미만에서 숨김, 대신 `@chat`의 모바일 히어로가 프로필 정보를 노출)
...
6. 모바일(`web` 미만)에서는 채팅 스크롤 영역 최상단에 `MobileHero`(`@chat/_components/mobileHero.tsx`) 노출 — 프로필 이미지·소개·연락처·소셜 링크 + "대화 시작하기" CTA(채팅 영역으로 스크롤). `web:hidden`으로 데스크탑에서는 숨기고 `@side`가 대신 노출. 스크롤 로직은 `body.tsx`의 `scrollElementToTop` 헬퍼를 `scrollToLastQuestion`/`scrollToChat`이 공유
...
- `profileSwiper.tsx` — 프로필 이미지 카드 슬라이더 (Swiper EffectCards), `@side`·모바일 히어로 공용
- `contactInfo.tsx` — 연락처 마스킹/복사 버튼, `@side`·모바일 히어로 공용
- `socialLinks.tsx` — 깃허브·블로그·링크드인 링크 카드, `@side`·모바일 히어로 공용
...
- 경로 별칭: `@/*`는 프로젝트 루트에 매핑
- **문서화 주석은 JSDoc 형식 사용**: 컴포넌트·공유 함수·복잡한 유틸의 설명 주석은 JSDoc(`@param`, `@returns`, `@example` 등 태그) 형식으로 작성. 단 모든 태그를 매번 다 적을 필요는 없고, 자명한 코드에는 생략 가능 — 재사용되거나 의도·사용법이 한눈에 안 들어오는 경우에 한해 작성.
